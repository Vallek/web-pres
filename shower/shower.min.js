...
!(function (global) {
	var undef,
			DECL_STATES = { NOT_RESOLVED: "NOT_RESOLVED", IN_RESOLVING: "IN_RESOLVING", RESOLVED: "RESOLVED" },
			create = function () {
					var curOptions = { trackCircularDependencies: !0, allowMultipleDeclarations: !0 },
							modulesStorage = {},
							waitForNextTick = !1,
							pendingRequires = [],
							define = function (name, deps, declFn) {
									...
									module || (module = modulesStorage[name] = { name: name, decl: undef }), (module.decl = { name: name, prev: module.decl, fn: declFn, state: DECL_STATES.NOT_RESOLVED, deps: deps, dependents: [], exports: undef });
							},
							require = function (modules, cb, errorCb) {
									"string" == typeof modules && (modules = [modules]),
											waitForNextTick || ((waitForNextTick = !0), nextTick(onNextTick)),
							...
							applyRequires = function () {
									var require,
											requiresToProcess = pendingRequires,
											i = 0;
					...
					return { create: create, define: define, require: require, getState: getState, isDefined: isDefined, setOptions: setOptions, getStat: getStat };
			},
			onError = function (e) {
					nextTick(function () {
							throw e;
			...
			nextTick = (function () {
					...
					if (global.postMessage && !global.opera) {
							...
							if (isPostMessageAsync) {
									...
									return (
											global.addEventListener ? global.addEventListener("message", onMessage, !0) : global.attachEvent("onmessage", onMessage),
											function (fn) {
	...
	(function (global) {
			function initialize() {
					global.shower.modules.require("shower.defaultOptions", function (defaultOptions) {
							...
							("false" !== getDataAttr("auto-init") || (hasOptions && autoInit)) &&
									(hasOptions ||
			...
			(global.shower = { modules: modules.create(), options: global.showerOptions || {} }), /interactive|complete|loaded/.test(document.readyState) ? initialize() : document.addEventListener("DOMContentLoaded", initialize);
	})(window),
	...
	shower.modules.define("Emitter", ["emitter.Event", "emitter.EventGroup", "util.extend"], function (provide, EmitterEvent, EventGroup, extend) {
			...
			extend(EventEmitter.prototype, {
					on: function (types, callback, context, priority) {
							...
							else for (var i = 0, l = types.length; i < l; i++) this._addListener(types[i], callback, context, priority);
							return this;
					},
					off: function (types, callback, context, priority) {
							...
							else for (var i = 0, l = types.length; i < l; i++) this._removeListener(types[i], callback, context, priority);
							return this;
					},
					...
					emit: function (eventType, eventObject) {
							...
							(event && "function" == typeof event.get) || (event = this.createEventObject(eventType, eventObject, this._context)),
									event.isPropagationStopped() || (listeners.hasOwnProperty(eventType) && this._callListeners(listeners[eventType], event), this._parent && !event.isPropagationStopped() && this._parent.emit(eventType, event));
					},
	...
	shower.modules.define("emitter.EventGroup", ["util.extend"], function (provide, extend) {
			...
			extend(EventGroup.prototype, {
					on: function (types, callback, context) {
							if (Array.isArray(types)) for (var i = 0, k = types.length; i < k; i++) this._listeners.push(types[i], callback, context);
					...
					off: function (types, callback, context) {
							if (Array.isArray(types)) for (var i = 0, k = types.length; i < k; i++) this._removeListener(types[i], callback, context);
					...
					_removeListener: function (type, callback, context) {
							for (var index = this._listeners.indexOf(type, 0); index != -1; )
									this._listeners[index + 1] == callback && this._listeners[index + 2] == context && (this._listeners.splice(index, 3), this.events.off(type, callback, context)), (index = this._listeners.indexOf(type, index));
					},
			}),
					provide(EventGroup);
	}),
			...
			extend(Plugins.prototype, {
					...
					_instancePlugin: function (plugin) {
							this._showerInstances.forEach(function (shower) {
									...
									this.events.emit("add", { name: plugin.name });
					},
	...
	shower.modules.define("Options", ["Emitter", "options.Monitor", "util.Store", "util.extend", "util.inherit"], function (provide, EventEmitter, Monitor, Store, extend, inherit) {
			...
			inherit(Options, Store, {
					set: function (name, value) {
							var changed = [];
							if ("string" == typeof name) Options.super.set.call(this, name, value), changed.push({ name: name, value: value });
							else {
									var options = name || {};
			...
			extend(Monitor.prototype, {
					...
					_onOptionsChange: function (event) {
							...
							fieldsUpdated.forEach(function (field) {
									this._fieldsHanders.hasOwnProperty(field) &&
	...
	shower.modules.define("Shower", ["Emitter", "Options", "shower.global", "shower.defaultOptions", "shower.Container", "shower.Player", "shower.Location", "shower.slidesParser", "util.extend"], function (
			provide,
			EventEmitter,
			Options,
			showerGlobal,
			defaultShowerOptions,
			Container,
			Player,
			Location,
			defaultSlidesParser,
			...
			function Shower(container, options) {
					...
					"string" == typeof containerElement && (containerElement = document.querySelector(containerElement)),
							...
							this.options.get("debug_mode") && (document.body.classList.add(this.options.get("debug_mode_classname")), console.info("Debug mode on")),
			...
			extend(Shower.prototype, {
					...
					add: function (slide) {
							if (Array.isArray.call(null, slide)) for (var i = 0, k = slide.length; i < k; i++) this._addSlide(slide[i]);
	...
	shower.modules.define("shower.Container", ["Emitter", "util.bound", "util.extend"], function (provide, EventEmitter, bound, extend) {
			function Container(shower, containerElement) {
					(this.events = new EventEmitter({ context: this, parent: shower.events })), (this._shower = shower), (this._element = containerElement), (this._isSlideMode = !1), this.init();
			}
			extend(Container.prototype, {
					...
					_setupListeners: function () {
							(this._showerListeners = this._shower.events.group().on("slideadd", this._onSlideAdd, this).on("slideremove", this._onSlideRemove, this)),
									...
									window.addEventListener("wheel", bound(this, "_onWheel"));
					},
					_clearListeners: function () {
							this._showerListeners.offAll(), window.removeEventListener("resize", bound(this, "_onResize")), document.removeEventListener("keydown", bound(this, "_onKeyDown")), window.removeEventListener("wheel", bound(this, "_onWheel"));
					},
	...
	shower.modules.define("shower.Location", ["util.SessionStore", "util.bound", "util.extend"], function (provide, SessionStore, bound, extend) {
			function Location(shower) {
					...
					(this.state = new SessionStore(sessionStoreKey, { isSlideMode: !1 })),
			...
			extend(Location.prototype, {
					...
					_init: function () {
							...
							(window.location.hash = ""),
									...
									"" !== currentSlideId && ((slideInfo = this._getSlideById(currentSlideId)), shower.player.go("undefined" != typeof slideInfo.index ? slideInfo.index : 0));
					},
					_setupListeners: function () {
					...
					_onPopstate: function () {
							...
							this._isSlideMode() && currentSlideNumber === -1 ? shower.player.go(0) : currentSlideNumber === -1 && "" !== window.location.hash && shower.player.go(0),
									currentSlide && slideId !== currentSlide.getId() && ((slideInfo = this._getSlideById(slideId)), shower.player.go(slideInfo.index));
					},
					_setTitle: function () {
	...
	shower.modules.define("shower.Player", ["Emitter", "util.bound", "util.extend"], function (provide, EventEmitter, bound, extend) {
			function Player(shower) {
					(this.events = new EventEmitter({ context: this, parent: shower.events })),
							(this._shower = shower),
							(this._showerListeners = null),
							(this._playerListeners = null),
			...
			extend(Player.prototype, {
					...
					go: function (index) {
							...
							return (
									index != this._currentSlideNumber &&
											index < slidesCount &&
											index >= 0 &&
											(currentSlide && currentSlide.isActive() && currentSlide.deactivate(),
											...
											this.events.emit("activate", { index: index, slide: currentSlide })),
									this
							);
					},
	...
	shower.modules.define("Slide", ["shower.defaultOptions", "Emitter", "Options", "slide.Layout", "slide.layoutFactory", "util.Store", "util.extend"], function (
			provide,
			defaultOptions,
			EventEmitter,
			OptionsManager,
			...
			function Slide(content, options, state) {
					(this.events = new EventEmitter()),
							...
							(this.layout = null),
							(this.state = new DataStore({ visited: 0, index: null }, state)),
			...
			extend(Slide.prototype, {
					init: function () {
							(this.layout = "string" == typeof this._content ? new slideLayoutFactory.createLayout({ content: this._content }) : new Layout(this._content, this.options)), this.layout.setParent(this), this._setupListeners();
					},
					destroy: function () {
	...
	shower.modules.define("slide.Layout", ["Options", "shower.defaultOptions", "Emitter", "util.bound", "util.extend"], function (provide, OptionsManager, defaultOptions, EventEmitter, bound, extend) {
			function Layout(element, options) {
					(this.options = new OptionsManager(
							{ title_element_selector: defaultOptions.slide_title_element_selector, active_classname: defaultOptions.slide_active_classname, visited_classname: defaultOptions.slide_visited_classname },
							options
					)),
							(this.events = new EventEmitter()),
			...
			extend(Next.prototype, {
					...
					_setupListeners: function () {
							...
							timerPlugin
									...
									: (this._pluginsListeners = globalShower.plugins.events.group().on(
												"add",
												function (e) {
			...
			extend(Timer.prototype, {
					...
					_setupListeners: function () {
							...
							this.events.on("next", this._onNext, this),
									...
									this._nextPlugin ||
											(this._pluginsListeners = shower.plugins.events.group().on(
													"pluginadd",
													function (e) {
					...
					_onSlideActivate: function () {
							...
							if (this._shower.container.isSlideMode() && currentSlide.state.get("visited") < 2) {
									...
									timing &&
											/^(\d{1,2}:)?\d{1,3}$/.test(timing) &&
											(timing.indexOf(":") !== -1 ? ((timing = timing.split(":")), (timing = 1e3 * (60 * parseInt(timing[0], 10) + parseInt(timing[1], 10)))) : (timing = 1e3 * parseInt(timing, 10)),
